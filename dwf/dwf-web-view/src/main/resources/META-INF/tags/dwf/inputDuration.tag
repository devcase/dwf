...
<dwf:resolveEL el="${entityName}.${attrMap.unitProperty}" var="unitValue" />
...
<dwf:formGroup parentAttrMap="${attrMap}">
	<div class="row">
 		<div class="col-xs-6 col-sm-4 col-md-3">
	 		<input type="text" value="<fmt:formatNumber value='${value}' pattern='${empty value ? "": "0.00"}' />" name="${name}"
			decimalSeparator="${decimal}" groupingSeparator="${grouping}"
			<c:if test="${attrMap.required}">required="required"</c:if>
			class='form-control number-mask <c:if test="${attrMap.required}">required</c:if>' />
		</div>
