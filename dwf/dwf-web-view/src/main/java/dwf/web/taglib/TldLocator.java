...
public class TldLocator {
	public HashMap<String, TagLibraryInfo> createTldInfos() throws IOException {
		...
		//dwf.tld
		String[] dwfTlds = new String[] {"META-INF/dwf.tld", "META-INF/decorator.tld"};
		
		for (String tld : dwfTlds) {
			InputStream is = null;
			try {
				URL url = loader.getResource(tld);
				is = loader.getResourceAsStream(tld);
				XMLStreamReader xmlReader = inputFactory.createXMLStreamReader(is);
				TldMetaData tldMetadata = TldMetaDataParser.parse(xmlReader);
				TagLibraryInfo taglibInfo = getTagLibraryInfo(tldMetadata);
				taglibInfo.setLocation(url.toString());
				if (!tagLibInfos.containsKey(taglibInfo.getUri())) {
					tagLibInfos.put(taglibInfo.getUri(), taglibInfo);
				}
			} catch (XMLStreamException e) {
		...
		for (Map.Entry<String, TagLibraryInfo> url : tagLibInfos.entrySet()) {
