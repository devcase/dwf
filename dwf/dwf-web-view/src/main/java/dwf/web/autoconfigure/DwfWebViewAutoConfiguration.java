...
import dwf.web.taglib.TldLocator;
...
//	}
	...
	@Bean
	public ServletRegistrationBean jastowServletRegistrationBean() {
		ServletRegistrationBean registration = new ServletRegistrationBean(
				new JspServlet(), "*.jsp");
		registration.setName("JSP Servlet");
		return registration;

	}
	
	/**
	 * Faz a mesma coisa que {@link io.undertow.jsp.JspServletBuilder.setupDeployment(DeploymentInfo, Map<String, JspPropertyGroup>, Map<String, TagLibraryInfo>, InstanceManager)}
	 * @return
	 */
	@Bean
	public ServletContextInitializer jastowAttributesServletContextInitializer() {
		return new ServletContextInitializer() {
			
			@Override
			public void onStartup(ServletContext servletContext) throws ServletException {
				TldLocator tldLocator = new TldLocator();
				HashMap<String, TagLibraryInfo> jspTagLibraries;
				try {
					jspTagLibraries = tldLocator.createTldInfos();
				} catch (IOException e) {
					throw new ServletException("Erro ao buscar as Taglibs");
				}
				servletContext.setAttribute(org.apache.jasper.Constants.JSP_TAG_LIBRARIES, jspTagLibraries);
				
				
				servletContext.setAttribute(org.apache.jasper.Constants.JSP_PROPERTY_GROUPS, new HashMap<String, JspPropertyGroup>());
//		        deploymentInfo.addServletContextAttribute(SERVLET_VERSION, deploymentInfo.getMajorVersion() + "." + deploymentInfo.getMinorVersion());
				servletContext.setAttribute(InstanceManager.class.getName(), new HackInstanceManager());
