<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
<%@ taglib uri="http://dwf.devcase.com.br/dwf" prefix="dwf"%>
<%@taglib uri="http://www.springframework.org/tags" prefix="spring"%>
<dwf:resolveEL el="${entityName}" var="entity"/>
<c:set var="formLayout" value="horizontal" scope="request"/>
<h1><spring:message code="label.editForm.header.${empty entity.id ? 'create' : 'edit'}"/> <spring:message code="domain.${entityName}"/></h1>
<div class="panel panel-default">
	<div class="panel-body">
		<form class="form-horizontal validate" method="POST" action="${appPath}/${entityName}/save" role="form">
			<input type="hidden" name="id" value="${entity.id}"/>
			<jsp:doBody />
			<div class="text-right">
				<button type="submit" class="btn btn-primary" data-loading-text="<spring:message code="action.wait"/>" formaction="${appPath}/${entityName}/save">
			 		<span class="glyphicon glyphicon-floppy-disk"></span>
			 		<spring:message code="action.save"/>
				</button>
			</div>
		</form>
	</div><!-- /.box-content -->
</div>
<c:set var="formLayout" value="" scope="request"/>